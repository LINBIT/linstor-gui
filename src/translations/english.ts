...
org_role = Viewer`,
    ...
    controller_auth_description:
      'Manage the LINSTOR controller Bearer token cached by this browser and sent with controller API requests when token authentication is enabled.',
    controller_auth_storage_title: 'Browser Token Cache',
    controller_auth_storage_description:
      'The token entered here is cached in this browser localStorage so the GUI can send it with controller requests. Token authentication itself is configured and validated on the LINSTOR controller.',
    controller_auth_initialize: 'Initialize Token Auth',
    controller_auth_initialize_help:
      'Enable controller token authentication, create the initial token, and store the returned token in this browser. This uses no_https=true so the current GUI session stays on HTTP.',
    controller_auth_initialized: 'Controller token authentication initialized and token stored in this browser.',
