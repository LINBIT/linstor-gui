// SPDX-License-Identifier: GPL-3.0
//
// Copyright (c) 2024 LINBIT
//
// Author: Liang Li <liang.li@linbit.com>

const en = {
  common: {
    // DRBD Reactor / OCF agent editor
    preview: 'Preview',
    paste: 'Paste',
    services: 'Services',
    metadata: 'Metadata',
    parameters: 'Parameters',
    provider: 'Provider',
    platform: 'Platform',
    description: 'Description',
    drbd_reactor_config: 'DRBD Reactor Config',
    add_metadata: 'Add Metadata',
    add_configuration: 'Add Configuration',
    live_preview_toml: 'Live Preview (TOML)',
    generating_preview: 'Generating preview...',
    systemd_unit_configuration: 'Systemd Unit Configuration',
    parsed_parameters: 'Parsed Parameters:',
    metadata_not_found: 'Metadata not found',
    loading_metadata: 'Loading metadata...',
    no_profile_selected: 'No profile selected',
    paste_toml_hint: 'Paste a full DRBD Reactor TOML configuration below to overwrite the current editor content.',
    configuration_applied: 'Configuration applied',
    failed_to_parse_configuration: 'Failed to parse configuration content',
    please_fix_validation_errors: 'Please fix validation errors',
    resource_name_required: 'Resource name is required',
    file_path_required: 'File path is required',
    failed_to_save: 'Failed to save: {{message}}',
    agent_removed: 'Agent removed',
    parameter_added: 'Parameter {{name}} added',
    parameter_removed: 'Parameter {{name}} removed',
    please_select_parameter: 'Please select a parameter',
    agent_metadata_not_found: 'Agent metadata not found',
    please_select_provider_and_agent: 'Please select provider and agent',
    added_ocf_agent: 'Added OCF agent: {{agent}}',
    please_enter_unit_name: 'Please enter a unit name',
    added_service_unit: 'Added service unit: {{name}}',
    added_mount_unit: 'Added mount unit: {{name}}',
    missing_key: 'Missing key',
    missing_value: 'Missing value',
    no_description_available: 'No description available',
    empty_value: '(empty)',
    required_label: 'required',
    unit_name_required: 'Unit name is required',
    param_is_required: '{{name}} is required',
    select_field: 'Select {{field}}',
    examples_service_unit: 'Examples: mysql.service or mysql',
    examples_mount_unit: 'Examples: var-lib-mysql.mount or var-lib-mysql',
    performance_metrics: 'Performance Metrics',
    no_grafana_panels_configured_check: 'No Grafana panels configured. Please check your Grafana settings.',
    add_resource_agent: 'Add Resource Agent',
    select_platform: 'Select a platform',
    filter_platform: 'Filter by platform',
    search_agents_provider_name_description: 'Search agents by provider, name or description...',
    add_parameter: 'Add Parameter',
    parameter: 'Parameter',
    select_parameter_add: 'Select parameter to add',
    default_value: 'Default Value',
    enter_values: 'Enter values',
    remove_field: 'Remove field',
    key: 'Key',
    drag_resize: 'Drag to resize',
    select_resource: 'Select resource',
    config_file: 'Config File',
    no_ocf_agents_found_in_start_array: 'No OCF agents found in start array',
    add_systemd_mount: 'Add Systemd/Mount',
    unit_type: 'Unit Type',
    unit_name: 'Unit Name',
    paste_toml_configuration: 'Paste TOML Configuration',
    systemd_unit: 'Systemd Unit',
    delete_item: 'Delete this item?',
    please_input_property_name: 'Please input property name',
    please_input_property_value: 'Please input property value',
    select_property: 'Select a property',
    property_editor: 'Property Editor',
    enter_column_data_index: 'Enter column data index',
    enter_column_title: 'Enter column title',
    please_input_name_my_export: 'Please input name: my_export',
    please_input_export_path: 'Please input export path: /',
    replica_count: 'Replica count:',
    please_input_pool_name: 'Please input pool name',
    add_create_new_pool: 'Add or create new pool',
    storage_pool_type: 'Storage Pool Type',
    grow_volume: 'Grow volume',
    delete_iscsi_target: 'Delete the ISCSI target',
    are_you_sure_delete_iscsi_target: 'Are you sure to delete this ISCSI target?',
    delete_nfs_target: 'Delete the NFS target',
    are_you_sure_delete_nfs_target: 'Are you sure to delete this NFS target?',
    color_coding: 'Color Coding',
    update_node: 'Update node?',
    are_you_sure_you_want_update_node: 'Are you sure you want to update this node?',
    warning: 'Warning',
    refresh_interval: 'Refresh Interval',
    system_info: 'System Info',
    linstor_gui_info: 'LINSTOR GUI Info',
    hci_host: 'HCI Host',
    are_you_sure_you_want_clear_all_logs: 'Are you sure you want to clear all logs?',
    color_theme: 'Color theme',
    delete_property: 'Delete the property',
    are_you_sure_delete_property: 'Are you sure to delete this property?',
    delete_network_interface: 'Delete the network interface',
    are_you_sure_delete_network_interface: 'Are you sure to delete this network interface?',
    are_you_sure_set_network_interface_as: 'Are you sure to set this network interface as active?',
    edit_storage_pool: 'Edit Storage Pool',
    edit_resource_group: 'Edit Resource Group',
    about_linbit_vsan: 'About LINBIT VSAN',
    vsan_host: 'VSAN Host',
    nfs_export_configuration: 'NFS Export Configuration',
    nvme_target_configuration: 'NVMe-oF Target Configuration',
    physical_storage: 'Physical Storage',
    resource_groups: 'Resource groups',
    linstor_reactor_create: 'LINSTOR | Reactor | Create',
    linstor_reactor_edit: 'LINSTOR | Reactor | Edit',
    disconnected: 'DISCONNECTED',
    connected: 'CONNECTED',
    on: 'ON',
    off: 'OFF',
    previous: 'Previous',
    skip: 'Skip',
    search: 'Search',
    regex_filter_hint: 'Supports regular expressions, matched case-insensitively (e.g. "dev.*").',
    property: 'Properties',
    add: 'Add',
    create: 'Create',
    reload: 'Reload',
    created: 'Created Time',
    view: 'View',
    edit: 'Edit',
    edit_mode: 'Edit Mode',
    migrate: 'Migrate',
    delete: 'Delete',
    lost: 'Lost',
    nodes: 'Nodes',
    resources: 'Resources',
    volumes: 'Volumes',
    error_reports: 'Error Reports',
    disk_creation_records: 'Disk creation records',
    deploy: 'Deploy',
    submit: 'Submit',
    cancel: 'Cancel',
    ok: 'OK',
    close: 'Close',
    snapshot: 'Snapshot',
    success: 'Success',
    submitted_processing: 'Submitted. Processing…',
    reset: 'Reset',
    name: 'Name',
    action: 'Action',
    resource_group: 'Resource Group',
    controller: 'Controller',
    add_property: 'Add Property',
    controller_properties: 'Controller Properties',
    back: 'Back',
    activate: 'Set as active',
    detail: 'Detail',
    storage_pool: 'Storage Pool',
    node: 'Node',
    spawn: 'Spawn',
    spawn_on_create: 'Spawn on create',
    advanced: 'Advanced',
    resource_name: 'Resource Name',
    resource_name_placeholder: 'Please input resource name',
    select_resource_group: 'Select a resource group',
    resource_group_required: 'Please select a resource group',
    volume: 'Volume',
    size_required: 'Please input a size',
    partial: 'Partial',
    spawn_partial: 'Partial (spawn fewer volumes than volume groups)',
    add_volume_group: 'Add Volume Group',
    add_volume_group_success: 'Volume group added',
    add_volume_group_failed: 'Failed to add volume group',
    volume_number: 'Volume Number',
    volume_number_hint: 'Leave blank to auto-assign the next volume number.',
    auto: 'Auto',
    definition_only: 'Definition only',
    definition_only_hint:
      'Create only the resource definition and volume definitions (metadata). The resource is not deployed to any node — no storage is allocated and no DRBD devices are created.',
    spawn_partial_hint:
      'This resource group has {{count}} volume groups. Provide one size per volume group, or leave trailing sizes blank to spawn only the first volumes (partial).',
    port: 'Port',
    size: 'Size',
    state: 'State',
    place_count: 'Place Count',
    diskless_on_remaining: 'Diskless on remaining',
    replication_mode: 'Replication Mode',
    async: 'Asynchronous(A)',
    semi_sync: 'Memory synchronous(B)',
    sync: 'Synchronous(C)',
    resource_definition: 'Resource Definition',
    created_on: 'Created On',
    usage_status: 'Usage Status',
    connection_status: 'Connection Status',
    add_column: 'Add Column',
    column_data_index: 'Column Data Index',
    column_title: 'Column Title',
    reset_column: 'Reset Column',
    resource_definition_name: 'Resource Definition Name',
    allocate_method: 'Allocate Method',
    manual: 'Manual',
    device_path: 'Device Path',
    resource: 'Resource',
    upload: 'Upload',
    save: 'Save',
    settings: 'Settings',
    status: 'Status',
    confirm: 'Confirm',
    stop: 'Stop',
    start: 'Start',
    starting: 'Starting...',
    stopping: 'Stopping...',
    deleting: 'Deleting...',
    logout: 'Logout',
    user: 'User',
    password: 'Password',
    login: 'Login',
    username: 'Username',
    logs: 'Logs',
    log_detail: 'Log Detail',
    clear_all_logs: 'Clear All Logs',
    mark_all_as_read: 'Mark All as Read',
    back_to_logs: 'Back to Logs',
    mark_as_read: 'Mark as Read',
    primary: 'Primary',
    adjust: 'Adjust',
    clone: 'Clone',
    external_name: 'External Name',
    use_zfs_clone: 'Use ZFS Clone',
    toggle: 'Toggle',
    layers: 'Layers',
    volume_definition: 'Volume Definition',
    volume_number_short: 'V#',
    keyword: 'Keyword',
    action_short: 'A',
    resource_definition_properties: 'Resource Definition Properties',
    volume_definition_properties: 'Volume Definition Properties',
    storage_pool_overview: 'Storage Pool Overview',
    feature_unavailable: 'Feature unavailable',
    feature_requires_api_version: 'This feature requires LINSTOR REST API {{version}} or later.',
    showing_top_n_of_total_nodes: 'Showing top {{n}} of {{total}} nodes by used capacity',
    top_n_nodes_hint:
      'For performance on large clusters, this chart only shows the top {{n}} nodes by used capacity. See Inventory → Storage Pools for the full list.',
    free_capacity: 'Free Capacity',
    used_capacity: 'Used Capacity',
    faulty_resource: 'Faulty Resource',
    all_resources_are_healthy: 'All resources are healthy',
    restore_failed: 'Restore failed',
    resize: 'Resize',
    operation_success: 'Operation Successful',
    creating: 'Creating...',
    loading: 'Loading...',
    failed: 'Failed',
    unlock: 'Unlock',
    total_items: 'Total {{total}} items',
    unsaved_changes: 'Unsaved Changes',
    unsaved_changes_desc: 'You have unsaved changes. Are you sure you want to leave? Your changes will be lost.',
    yes: 'Yes',
    no: 'No',
    invalid_request: 'Invalid Request',
  },
  menu: {
    dashboard: 'Dashboard',
    support: 'Support',
    inventory: 'Inventory',
    node: 'Nodes',
    controller: 'Controller',
    storage_pools: 'Storage Pools',
    error_reports: 'Error Reports',
    software_defined: 'Storage Configuration',
    resource_overview: 'Resource Overview',
    resource_groups: 'Resource Groups',
    resource_definitions: 'Resource Definitions',
    volume_definitions: 'Volume Definitions',
    resources: 'Resources',
    volumes: 'Volumes',
    node_ip_addrs: 'IP Addresses',
    remotes: 'Remote',
    linstor: 'LINSTOR',
    s3: 'S3',
    settings: 'Settings',
    gateway: 'Gateway',
    iscsi: 'iSCSI',
    nfs: 'NFS',
    'nvme-of': 'NVMe-oF',
    snapshot: 'Snapshots',
    grafana: 'Grafana',
    users: 'Users',
    authentication: 'Authentication',
    'backup&dr': 'Backup / DR',
    schedule_list: 'Schedule',
    files: 'Files',
    auth_tokens: 'Auth Tokens',
    ha: 'HA',
    high_availability: 'High Availability',
    reactor: 'DRBD Reactor',
  },
  clusterSetup: {
    welcome_title: "Welcome — let's set up your LINSTOR cluster",
    welcome_description:
      'There are no nodes registered with this controller yet. Use the guided wizard to register your satellites and prepare storage pools.',
    welcome_bullet_nodes: 'Register one or more satellite nodes',
    welcome_bullet_pools: 'Create storage pools (existing or fresh devices)',
    get_started: 'Get started',
    title: 'Cluster setup',
    step_nodes: 'Add nodes',
    step_pools: 'Add storage pools',
    step_done: 'Done',
    nodes_hint:
      'Add one row per satellite. Each node will be registered with its address and port; you can adjust other settings later from the Nodes page.',
    pools_hint:
      'Choose whether to create a brand-new device pool (the controller will run pvcreate / vgcreate / etc. for you) or register an existing LVM / ZFS pool. The same pool is created on every node.',
    node_name: 'Name',
    node_ip: 'IP address',
    node_port: 'Port',
    node_type: 'Type',
    add_node_row: 'Add another node',
    pool_name: 'Storage pool name',
    provider: 'Provider',
    pool_mode: 'Source',
    mode_new_device: 'New device',
    mode_existing: 'Existing',
    source: 'Device / pool',
    pool_applies_to_all: 'This pool is created on every node: {{nodes}}',
    add_pool_row: 'Add another storage pool',
    device_path_help: 'Path of an empty block device on the satellite (e.g. /dev/sdb). The controller will prepare it.',
    existing_help: 'Name of an existing VG / Thin pool / ZFS pool already provisioned on the satellite.',
    save_and_continue: 'Save & continue',
    finish: 'Finish',
    required: 'Required',
    add_at_least_one_node: 'Add at least one node.',
    no_nodes_created: 'No nodes could be created. Fix the errors and try again.',
    partial_node_success: '{{ok}} of {{total}} nodes created. Continuing with the successful ones.',
    nodes_created: '{{count}} node(s) created.',
    partial_pool_success: '{{ok}} of {{total}} storage pools created.',
    pools_created: '{{count}} storage pool(s) created.',
    all_done: 'Cluster set up successfully',
    done_description:
      'Your nodes and storage pools are registered. You can now create resources and manage replication from the rest of the GUI.',
    dismiss: 'Dismiss',
    step_resource_group: 'Resource group',
    rg_hint:
      'Optional. Create one resource group now, or skip to finish. Picking a preset pre-fills sensible defaults for common workloads — you can still tweak each field.',
    preset_label: 'Preset',
    preset_placeholder: '— none —',
    preset_vm: 'VM (general)',
    preset_proxmox: 'Proxmox VE',
    preset_cloudstack: 'CloudStack',
    preset_ha: 'High availability',
    rg_name: 'Resource group name',
    place_count: 'Place count',
    replicas_on_different: 'Replicas on different',
    replicas_on_same: 'Replicas on same',
    drbd_options_header: 'DRBD options',
    auto_promote: 'auto-promote',
    net_protocol: 'Net/protocol',
    resource_quorum: 'Resource/quorum',
    on_no_quorum: 'Resource/on-no-quorum',
    on_suspended_primary_outdated: 'Resource/on-suspended-primary-outdated',
    disk_flushes: 'Disk/disk-flushes',
    next: 'Next',
    step_review: 'Review & create',
    review_hint:
      'Review what will be created. Nothing has been sent to the controller yet — it is all created when you click Create cluster.',
    further_tasks: 'Further LINSTOR tasks',
    further_tasks_intro:
      'Once the cluster is up, you may want to make the controller itself highly available so a single node going down does not take LINSTOR with it.',
    ha_guide_label: 'Set up a highly available LINSTOR controller',
    ha_cmd_intro:
      'linstor-controller-ha-setup ships with linstor-controller. Copy the command below, log in to the node that is currently running linstor-controller, and run it there. It moves the controller database onto a replicated DRBD resource and hands the controller over to DRBD Reactor — when it finishes, it prints the commands to run on each standby node.',
    ha_cmd_req_root: 'Run as root on the active controller node — that node must be one of the candidates below.',
    ha_cmd_req_packages: 'Requires the linstor CLI, drbd-utils and drbd-reactor on that node.',
    ha_cmd_req_standby: 'Standby nodes need linstor-controller and drbd-reactor installed.',
    ha_cmd_nodes_label: 'Controller candidate nodes',
    ha_cmd_nodes_hint:
      'Only these nodes get a controller and a replica of its database. Three is the usual choice; more nodes mainly add replication overhead.',
    ha_cmd_nodes_min: 'Pick at least {{count}} nodes — the controller cannot fail over to itself.',
    ha_cmd_vip_label: 'Controller virtual IP (optional)',
    ha_cmd_vip_hint:
      'Leave empty to point clients at the list of candidates instead. With a VIP, DRBD Reactor brings the address up on whichever node runs the controller, so clients and integrations only need one address.',
    ha_cmd_vip_invalid: 'Enter the address with a prefix length, for example 10.0.0.100/24.',
    ha_cmd_copy: 'Copy command',
    ha_cmd_copied: 'Command copied to clipboard',
    ha_cmd_copy_failed: 'Copy failed — select the command text and copy it manually',
    ha_cmd_outro:
      'The script asks for confirmation before changing anything and aborts if the cluster already looks HA-configured. Afterwards, point clients and integrations (Proxmox, CSI, …) at all controller IPs — see the LINSTOR user guide for the full reference.',
    create_cluster: 'Create cluster',
    creating_cluster: 'Creating cluster…',
    pool_skipped_node_failed: 'skipped — node was not created',
    rg_skipped_none: 'None',
    service_label: 'Service',
    service_placeholder: 'Select a service',
    service_hint:
      'Pick a service to pre-fill recommended DRBD properties below. You can still edit, add, or remove any property.',
    service_vm: 'VM',
    service_ha: 'HA',
    properties_header: 'Properties',
    property_key: 'Property',
    property_value: 'Value',
    add_property: 'Add property',
    create_resource_group: 'Create resource group',
    rg_created: 'Resource group {{name}} created.',
    rg_failed: 'Failed to create resource group: {{error}}',
    rg_skipped: 'Resource group step skipped.',
    ha_needs_three_nodes: 'The HA preset assumes at least three nodes for quorum. The current cluster has {{count}}.',
  },
  authToken: {
    title: 'Auth Tokens',
    token_visibility_notice:
      'Token values are only returned when they are created. This page only shows token metadata.',
    refresh: 'Refresh',
    load_failed: 'Failed to load auth tokens.',
    id: 'ID',
    description: 'Description',
    created: 'Created',
    active: 'Active',
    user_token: 'UserToken',
    ip_filter: 'IP Filter',
    expires: 'Expires',
    total_items: 'Total {{total}} items',
    actions: 'Actions',
    create: 'Create Token',
    description_required: 'Please enter a description.',
    ip_filter_help: 'Optional. Restrict this token to one client IPv4 or IPv6 address.',
    expires_help: 'Optional expiration date and time.',
    create_success: 'Auth token created successfully.',
    create_failed: 'Failed to create auth token.',
    create_missing_token: 'Token was created but no token value was returned.',
    created_token_title: 'New Auth Token',
    created_token_notice: 'Copy this token now. The controller will not return the token value again.',
    enable: 'Enable',
    disable: 'Disable',
    disable_confirm: 'Disable this auth token?',
    enable_success: 'Auth token enabled.',
    disable_success: 'Auth token disabled.',
    update_failed: 'Failed to update auth token.',
    delete: 'Delete',
    delete_confirm: 'Delete this auth token?',
    delete_success: 'Auth token deleted.',
    delete_failed: 'Failed to delete auth token.',
    cancel: 'Cancel',
    close: 'Close',
    show_all: 'Show all tokens',
    auth_disabled_title: 'Token Authentication is Disabled',
    auth_disabled_description:
      'Token authentication must be enabled on the controller before you can manage auth tokens. Open Settings and initialize it under the Controller Auth tab.',
    go_to_settings: 'Go to Settings',
  },
  node: {
    please_input_node_name: 'Please input node name',
    please_select_node_type: 'Please select node type',
    please_input_port: 'Please input port',
    delete_node: 'Delete the node',
    are_you_sure_delete_node: 'Are you sure to delete this node?',
    lost_node: 'Lost the node',
    are_you_sure_lost_node: 'Are you sure to lost this node?',
    delete_nodes: 'Delete nodes',
    are_you_sure_delete_selected_nodes: 'Are you sure to delete selected nodes?',
    lost_nodes: 'Lost nodes',
    are_you_sure_lost_selected_nodes: 'Are you sure to lost the selected nodes?',
    node_list: 'Node List',
    node_name: 'Name',
    platform: 'Platform',
    default_ip: 'IP',
    default_port: 'Port',
    node_type: 'Type',
    node_status: 'Status',
    node_detail: 'Node Detail',
    create_node: 'Create Node',
    edit_node: 'Edit Node',
    lost_tooltip_offline_required: 'Only offline nodes can be marked as lost',
  },
  node_detail: {
    title: 'Node Detail',
    node_name: 'Node name',
    platform: 'Platform',
    os_variant: 'OS variant',
    node_type: 'Node type',
    node_status: 'Node status',
    resource_layers: 'Resource layers',
    storage_providers: 'Storage providers',
    network_interfaces: 'Network interfaces',
    storage_pool_info: 'Storage pool info',
    resource_info: 'Resource info',
    add_network_interface: 'Add Network Interface',
    create_network_interface: 'Create network interface',
    alias: 'Alias',
    ip_address: 'IP Address',
    tcp_port: 'TCP Port',
    encryption_type: 'Encryption Type',
    default_ip: 'Default IP',
    plain: 'Plain',
    ssl: 'SSL',
    please_input_alias: 'Please input alias',
    please_input_ip_address: 'Please input IP address',
    please_input_tcp_port: 'Please input TCP port',
    please_input_valid_ip_address: 'Please input valid IP address',
    please_input_valid_port: 'Please input valid port, 0-65535',
    valid_ip_tooltip: 'Must be valid IP address, like 192.168.1.1, 10.10.1.1',
    valid_port_tooltip: 'Must be valid port',
    encryption_tooltip:
      'If satellite port and satellite encryption type are given the netinterface can also work as connection to the controller',
    network_interface_used_by: 'Network interface {{name}} is used by:',
    nodes: 'Nodes:',
    resource_group: 'Resource Group:',
    current_node: '(current node)',
    not_used_by_resource_group: 'Not used by any resource group',
    loading: 'Loading...',
  },
  storage_pool: {
    enter_storage_pool_name: 'Enter storage pool name',
    select_node_nodes: 'Select the node or nodes',
    select_path_physical_device: 'Select the path of the physical device',
    enter_pool_name: 'Enter pool name',
    vdo_slab_size: 'VDO Slab Size',
    vdo_logical_size: 'VDO Logical Size',
    please_input_storage_pool_name: 'Please input storage pool name',
    please_select: 'Please select',
    delete_storage_pool: 'Delete the storage pool',
    are_you_sure_delete_storage_pool: 'Are you sure to delete this storage pool?',
    delete_storage_pools: 'Delete storage pools',
    are_you_sure_delete_selected_storage: 'Are you sure to delete selected storage pools?',
    list: 'Storage Pool List',
    name: 'Name',
    node_name: 'Node Name',
    network_preference: 'Network Preference',
    disk: 'Disk',
    provider_kind: 'Provider Kind',
    capacity: 'Capacity',
    free_capacity: 'Free Capacity',
    total_capacity: 'Total Capacity',
    supports_snapshots: 'Supports Snapshots',
    show_default: 'Show Default',
    add_title: 'Add Storage Pool',
    add_description:
      'A storage pool identifies physically-backed storage that LINSTOR storage volumes consume. You can assign a storage pool to other LINSTOR objects, such as LINSTOR resources, resource definitions, or resource groups. LINSTOR storage volumes created from these objects will consume physical storage from the storage pool assigned to the object.',
    new_device: 'New Device',
    new_device_description: 'When creating storage pool using new device, please make sure the device is empty.',
    existing_device: 'Existing Device',
    existing_device_description:
      'To use an existing device, first create a volume group and logical volume by using LVM CLI commands.',
    storage_pool_name: 'Storage Pool Name',
    node: 'Node',
    type: 'Type',
    lvm_pool: 'Volume Group/Thin Pool',
    zfs_pool: 'Pool Name',
    zfs_toast: 'You need to install and configure ZFS on the node before creating a ZFS storage pool.',
    multiple_nodes: 'Multiple Nodes',
    device_path: 'Device Path',
    device_path_tooltip:
      'Select the path of the physical device that will back the storage pool. The device should be empty.',
    advanced_options: 'Advanced Options',
    lvm_pool_name: 'LVM Pool Name',
    sed: 'SED Enabled',
    vdo: 'VDO Enabled',
    zfs_pool_name: 'ZFS Pool Name',
  },
  ip_address: {
    list: 'IP Addresses List',
    node: 'Node',
    ip_address: 'IP Address',
    tcp_port: 'TCP Port',
    alias: 'Alias',
    is_management_network: 'Active',
  },
  error_report: {
    please_select_node: 'Please select node',
    delete_error_report: 'Delete the error report',
    are_you_sure_delete_error_report: 'Are you sure to delete this error report?',
    please_select_module: 'Please select module',
    delete_error_reports: 'Delete the error reports',
    are_you_sure_delete_selected_error: 'Are you sure to delete selected error reports?',
    id: 'ID',
    time: 'Time',
    module: 'Module',
    content: 'Content',
    detail_title: 'Error Report Detail',
    list_title: 'Error Report List',
    download_sos: 'Download SOS Report',
    time_range: 'Time Range',
  },
  dashboard: {
    title: 'Dashboard',
  },
  resource_group: {
    please_input_resource_group_name: 'Please input resource group name',
    please_input_description: 'Please input description',
    please_select_layers: 'Please select layers',
    please_select_providers: 'Please select providers',
    please_input_resource_definition_group: 'Please input resource definition group name',
    volume_size: 'Volume Size',
    delete_resource_group: 'Delete the resource group',
    are_you_sure_delete_resource_group: 'Are you sure to delete this resource group?',
    delete_selected_resource_groups: 'Delete selected resource groups',
    are_you_sure_delete_selected_resource: 'Are you sure to delete selected resource groups?',
    list: 'Resource Group List',
    name: 'Resource Group Name',
    place_count: 'Place Count',
    storage_pools: 'Storage Pool(s)',
    replication: 'Replication Mode',
    auto: 'Auto',
    async: 'Asynchronous(A)',
    semi_sync: 'Memory synchronous(B)',
    sync: 'Synchronous(C)',
    diskless: 'Diskless on remaining',
    auto_select_filter: 'Auto Select Filter',
    volume_numbers: 'Volume Numbers',
    properties: 'Properties',
    description: 'Description',
    create: 'Create Resource Group',
    storage_providers: 'Storage Providers',
    linstor_layers: 'LINSTOR Layers',
    drbd_protocol: 'DRBD Protocol',
    spawn_on_create: 'Spawn on create',
    storage_pool: 'Storage Pool',
    show_advanced: 'Show advanced setting',
    hide_advanced: 'Hide advanced settings',
    replicas_on_same: 'Replicas On Same',
    do_not_place_with: 'Do Not Place With',
    replicas_on_different: 'Replicas On Different',
    do_not_place_with_regex: 'Do Not Place With Regex',
  },
  resource_definition: {
    please_input_resource_definition_name: 'Please input resource definition name',
    are_you_sure_delete_resource_definition: 'Are you sure to delete this resource definition?',
    delete_resource_definition: 'Delete resource definition',
    are_you_sure_delete_selected_resource: 'Are you sure to delete selected resource definitions?',
    name: 'Name',
    list: 'Resource Definition List',
    resource_group_name: 'Resource Group Name',
    size: 'Size',
    port: 'Port',
    state: 'State',
    create: 'Create Resource Definition',
    edit: 'Edit Resource Definition',
  },
  volume_definition: {
    please_select_resource_definition: 'Please select resource definition',
    name: 'Name',
    list: 'Volume Definition List',
    resource_group_name: 'Resource Group Name',
    create: 'Create Volume Definition',
  },
  resource: {
    input_clone_name: 'Input clone name',
    please_select_resource_definition_group: 'Please select resource definition group',
    please_select_storage_pool: 'Please select storage pool',
    please_input_place_count: 'Please input place count',
    adjust_resource: 'Adjust the resource',
    are_you_sure_adjust_resource: 'Are you sure to adjust this resource?',
    delete_resource_definition: 'Delete the resource definition',
    are_you_sure_delete_resource: 'Are you sure to delete this resource definitions?',
    please_enable_configure_grafana: 'Please enable and configure Grafana Dashboard in Settings to view stats',
    toggle_resource: 'Toggle the resource',
    are_you_sure_toggle_resource: 'Are you sure to toggle this resource?',
    delete_resource: 'Delete the resource',
    are_you_sure_delete_resource_2: 'Are you sure to delete this resource?',
    please_input_snapshot_name: 'Please input snapshot name here...',
    list: 'Resource List',
    overview: 'Resource Overview',
    create: 'Create Resource',
    edit: 'Edit Resource',
    create_snapshot: 'Create Snapshot',
    migrate: 'Migrate Resource',
    resource: 'Resource',
    from: 'From Node',
    to: 'To Node',
    connection_status: 'Connections',
    add_disk: 'Add Disk',
    remove_disk: 'Remove Disk',
    search_placeholder: 'Search by resource name or aux property value',
    add_to_node: 'Add Resource to Node',
    diskful: 'Diskful',
    drbd_diskless: 'DRBD Diskless',
    storage_pool_required: 'Please select a storage pool',
  },
  volume: {
    list: 'Volume List',
    device_name: 'Device Name',
    allocated: 'Allocated',
    in_use: 'In Use',
    allocated_size: 'Allocated Size',
    reserved_size: 'Reserved Size',
    resource_volume: 'Resource/VolumeNumber',
  },
  controller: {
    controller_detail: 'Controller Detail',
  },
  // Remote / Linstor
  linstor: {
    list: 'LINSTOR List',
  },
  iscsi: {
    creates_highly_available_iscsi_target: 'Creates a highly available iSCSI target based on LINSTOR and DRBD Reactor.',
    please_select_resource_group: 'Please select resource group',
    gross_size: 'Gross Size',
    chap_username: 'CHAP username',
    chap_password: 'CHAP password',
    are_you_sure_delete_target: 'Are you sure to delete this target?',
    are_you_sure_delete_volume: 'Are you sure to delete this volume?',
    delete_targets: 'Delete targets',
    are_you_sure_delete_selected_targets: 'Are you sure to delete the selected targets?',
    list: 'iSCSI List',
    create: 'iSCSI Create',
    iqn: 'IQN',
    resource_group: 'Resource Group',
    service_ips: 'Service IP',
    on_node: 'On Node',
    linstor_state: 'LINSTOR State',
    service_state: 'Service State',
    lun: 'LUN',
    add_volume: 'Add Volume',
    delete_volume: 'Delete Volume',
    adding_volume: 'Adding Volume',
    deleting_volume: 'Deleting Volume',
    add_service_ip: 'Add Service IP',
  },
  nfs: {
    are_you_sure_delete_export: 'Are you sure to delete this export?',
    delete_exports: 'Delete exports',
    are_you_sure_delete_selected_exports: 'Are you sure to delete the selected exports?',
    list: 'NFS List',
    only_one_resource_note:
      'NOTE: With the Kernel NFS implementation, only one NFS resource can exist in a cluster (create one resource with multiple volumes). Use NFS-Ganesha to run multiple independent NFS resources.',
    implementation: 'NFS Server',
    implementation_kernel: 'Kernel',
    implementation_ganesha: 'NFS-Ganesha',
    implementation_help:
      'Kernel NFS (default) allows only one NFS resource per cluster. NFS-Ganesha supports multiple independent NFS resources, but requires nfs-ganesha and the ganesha-nfs resource agent installed on all nodes.',
    create_disabled_kernel:
      'A Kernel NFS resource already exists. Kernel NFS allows only one resource per cluster — delete it, or use NFS-Ganesha for multiple resources.',
    ganesha_requires_gateway: 'NFS-Ganesha requires linstor-gateway {{version}} or later.',
    create: 'NFS Create',
    name: 'Name',
    size: 'Size',
    service_ip: 'Service IP',
    on_node: 'On Node',
    export_path: 'Export Path',
    linstor_state: 'LINSTOR State',
    service_state: 'Service State',
    file_system: 'File System',
    volumes: 'Volumes',
    allowed_ips: 'Allowed IPs',
    volume_number: 'Volume #',
  },
  nvme: {
    creates_highly_available_nvme_target:
      'Creates a highly available NVMe-oF target based on LINSTOR and DRBD Reactor.',
    list: 'NVMe-oF List',
    nqn: 'NQN',
    create: 'NVMe-oF Create',
    resource_group: 'Resource Group',
    service_ip: 'Service IP',
    service_ips: 'Service IPs',
    on_node: 'On Node',
    linstor_state: 'LINSTOR State',
    service_state: 'Service State',
    lun: 'LUN',
    add_volume: 'Add Volume',
    delete_volume: 'Delete Volume',
    adding_volume: 'Adding Volume',
    deleting_volume: 'Deleting Volume',
    add_service_ip: 'Add Service IP',
  },
  snapshot: {
    please_input_snapshot_name: 'Please input snapshot name',
    please_select_resource: 'Please select resource',
    please_select_nodes: 'Please select nodes',
    delete_snapshots: 'Delete snapshots',
    are_you_sure_delete_selected_snapshots: 'Are you sure to delete selected snapshots?',
    list: 'Snapshot List',
    resource_name: 'Resource Name',
    node_names: 'Node Names',
    snapshot_name: 'Snapshot Name',
    volumes: 'Volumes',
    create: 'Create Snapshot',
    nodes: 'Nodes',
    create_failed: 'Create Snapshot Failed',
    create_success: 'Create Snapshot Success',
    rollback: 'Rollback',
    rollback_snapshot: 'Rollback Snapshot',
    rollback_confirmation_message: 'Are you sure you want to rollback resource {{resource}} to snapshot {{snapshot}}?',
    rollback_warning:
      'Warning: This operation will revert the resource to the state of the selected snapshot. Any data changes after the snapshot was created will be lost.',
    rollback_usage_warning: 'The resource must not be in use. That is, the resource must not be mounted on any nodes.',
    restore: 'Restore',
    restore_snapshot: 'Restore Snapshot',
    restore_success: 'Restore succeeded',
    restore_failed: 'Restore failed',
    creating_resource: 'Creating resource definition...',
    resource_created: 'Resource definition created successfully',
    resource_creation_failed: 'Failed to create resource definition',
    restoring_volume_definition: 'Restoring volume definition...',
    volume_definition_restored: 'Volume definition restored successfully',
    restoring: 'Restoring snapshot...',
    source_info: 'Source Information',
    source_resource: 'Source Resource',
    target_resource: 'Target Resource',
    select_source: 'Select source resource',
    select_snapshot: 'Select snapshot',
    select_target: 'Select target resource',
    resource: 'Resource',
    snapshot: 'Snapshot',
    new_resource_tip: 'If you enter a new resource name, it will be created automatically',
  },
  users: {
    please_check_username_password_try: 'Please check your username and password and try again',
    default_credential_admin_admin: 'Default credential: admin/admin',
    description: 'You can enable or disable user authentication from here.',
    authentication: 'User authentication',
    no_user: 'There are no users created yet.',
    add_a_user: 'Add a user',
    reset_password: 'Reset password',
    delete_user: 'Delete user',
    new_password: 'New password',
    confirm_password: 'Confirm password',
    change_password: 'Change password',
    current_password: 'Current password',
    dont_show_again: "Don't show this again",
    username: 'Username',
    password: 'Password',
    add: 'Add',
    password_changed: 'Password changed successfully',
    password_reset: 'Password reset successfully',
    password_change_failed: 'Password change failed',
    // UserManagement page
    title: 'Authentication & Users',
    delete_user_title: 'Delete the user',
    delete_user_description: 'Are you sure to delete this user?',
    removal_warning:
      "The 'Users' functionality will be removed in the future. Please use 'Auth Token' for access management and control.",
    authentication_enabled: 'Authentication is now enabled successfully',
    authentication_disabled: 'Authentication is now disabled successfully',
    authentication_update_failed: 'Failed to update the authentication status',
    user_description: 'User',
    switch_on: 'On',
    switch_off: 'Off',
    yes: 'Yes',
    no: 'No',
  },
  remote: {
    delete_backup: 'Delete this backup?',
    select_resource: 'Select a resource',
    please_input_name: 'Please input name',
    please_input_endpoint: 'Please input endpoint',
    please_input_bucket: 'Please input bucket',
    please_input_region: 'Please input region',
    please_input_access_key: 'Please input access key',
    please_input_secret_key: 'Please input secret key',
    please_input_url: 'Please input url',
    delete_remote_object: 'Delete this remote object?',
    select_type: 'Select type',
    list: 'Remote List',
    name: 'Name',
    type: 'Type',
    Info: 'Info',
    backups: 'Backups',
    endpoint: 'Endpoint',
    bucket: 'Bucket',
    region: 'Region',
    access_key: 'Access Key',
    secret_key: 'Secret Key',
    url: 'URL',
    use_path_style: 'Use Path Style',
    backup_count: 'Backup Count',
  },
  schedule: {
    cron_expression: 'Cron Expression',
    cron_editor: 'Cron Editor',
    invalid_cron_expression: 'Invalid Cron Expression',
    delete_schedule: 'Delete this schedule?',
    enter_schedule_name: 'Enter schedule name',
    full_cron_expression: 'Full Cron Expression',
    enter_full_cron_expression_e_g: 'Enter full cron expression (e.g., * * * * *)',
    incremental_cron_expression: 'Incremental Cron Expression',
    enter_incremental_cron_expression_e_g: 'Enter incremental cron expression (e.g., * * * * *)',
    keep_local_snapshots: 'Keep Local Snapshots',
    enter_number_local_snapshots: 'Enter number of local snapshots',
    keep_remote_backups: 'Keep Remote Backups',
    enter_number_remote_backups: 'Enter number of remote backups',
    max_retries: 'Max Retries',
    enter_max_retries_optional: 'Enter max retries (optional)',
    list: 'Schedule List',
    name: 'Name',
    type: 'Type',
    status: 'Status',
    time: 'Time',
    last_run_time: 'Last Run Time',
    next_run_time: 'Next Run Time',
    create: 'Create Schedule',
    edit: 'Edit Schedule',
    delete: 'Delete Schedule',
    schedule_name: 'Schedule Name',
    schedule_type: 'Schedule Type',
    schedule_status: 'Schedule Status',
    full_cron: 'Full',
    inc_cron: 'Incremental',
    keep_local: 'Keep Local',
    keep_remote: 'Keep Remote',
    on_failure: 'On Failure',
    // Schedule by resource page
    schedule_list: 'Schedule List',
    resource: 'Resource',
    remote: 'Remote',
    schedule: 'Schedule',
    last: 'Last',
    next: 'Next',
    planned_inc: 'Planned Inc',
    planned_full: 'Planned Full',
    reason: 'Reason',
    schedules: 'Schedules',
    show_all: 'Show All',
    enable: 'Enable',
    disable: 'Disable',
    enabled: 'Enabled',
    disabled: 'Disabled',
    resource_definition: 'Resource-Definition',
    resource_group: 'Resource-Group',
    controller: 'Controller',
    // Confirmation messages
    enable_schedule_confirm: 'Are you sure you want to enable this schedule?',
    disable_schedule_confirm: 'Are you sure you want to disable this schedule?',
    delete_schedule_confirm: 'Are you sure you want to delete this schedule?',
    delete_schedule_warning: 'This action cannot be undone!',
    // Success/Error messages
    schedule_enabled_success: 'Schedule enabled successfully',
    schedule_disabled_success: 'Schedule disabled successfully',
    schedule_deleted_success: 'Schedule deleted successfully',
    schedule_enable_failed: 'Failed to enable schedule',
    schedule_disable_failed: 'Failed to disable schedule',
    schedule_delete_failed: 'Failed to delete schedule',
    resource_details_load_failed: 'Failed to load resource details',
    // Enable Schedule Form
    enable_schedule: 'Enable Schedule',
    remote_name: 'Remote Name',
    resource_name: 'Resource Name',
    resource_group_name: 'Resource Group Name',
    node_name: 'Node Name',
    select_remote_name: 'Select remote name',
    select_schedule_name: 'Select schedule name',
    select_resource_name: 'Select resource name',
    select_resource_group_name: 'Select resource group name',
    select_node_name: 'Select node name',
    please_select_remote_name: 'Please select the remote name!',
    please_select_schedule_name: 'Please select the schedule name!',
    schedule_enabled_successfully: 'Schedule enabled successfully!',
    failed_to_enable_schedule: 'Failed to enable schedule',
    failed_to_fetch_schedule_list: 'Failed to fetch schedule list',
    failed_to_fetch_remote_list: 'Failed to fetch remote list',
    failed_to_fetch_resource_list: 'Failed to fetch resource list',
    failed_to_fetch_resource_group_list: 'Failed to fetch resource group list',
  },

  files: {
    title: 'External Files',
    description:
      'External files are managed by LINSTOR and can be deployed to resource definitions. They are commonly used to distribute configuration files for high availability setups, such as DRBD Reactor promoter configurations.',
    path: 'Path',
    content: 'Content',
    add_file: 'Add File',
    create: 'Create File',
    view_content: 'View Content',
    file_content: 'File Content',
    load_content_failed: 'Failed to load file content',
    delete_confirm: 'Are you sure to delete this file?',
    path_required: 'File path is required',
    content_required: 'File content is required',
    path_placeholder: '/etc/linstor.d/my-file.conf',
    content_placeholder: 'Enter file content...',
    no_content: 'No content available',
    deploy: 'Deploy',
    undeploy: 'Undeploy',
    undeploy_confirm: 'Are you sure to undeploy this file from its resource?',
    deploy_to_resource: 'Deploy to Resource',
    select_resource: 'Select Resource',
    select_resource_placeholder: 'Select a resource definition',
    deploy_success: 'File deployed successfully',
    deploy_failed: 'File deployment failed',
    undeploy_success: 'File undeployed successfully',
    undeploy_failed: 'File undeployment failed',
    modify: 'Modify',
    modify_file: 'Modify File',
    modify_success: 'File modified successfully',
    modify_failed: 'File modification failed',
    already_deployed: 'File is already deployed to this resource',
    already_deployed_to_all: 'File is already deployed to all resources',
    deployed_to: 'Deployed To',
    not_deployed: 'Not Deployed',
    show_reactor_files: 'Show DRBD Reactor configuration files',
    file: 'File',
  },

  settings: {
    unlock_linstor: 'Unlock LINSTOR',
    drbd_reactor_dashboard: 'DRBD Reactor Dashboard',
    current_logo: 'Current Logo',
    // Label above the Create/Edit radio group on the passphrase form. The
    // key was in use but had no English entry, so the raw key was rendered.
    create_edit_label: 'Create / Edit',
    title: 'Settings',
    general: 'General',
    custom_logo: 'Custom Logo',
    custom_logo_description:
      'You can select either a local SVG file or a remote URL. The URL can point to any image type. Please use a light-colored logo. A maximum space of 160x56 pixels is available for the logo.',
    logo: 'Logo',
    url: 'URL',
    gateway: 'Gateway',
    linstor_gateway: 'LINSTOR Gateway',
    linstor_gateway_description:
      'Manages Highly-Available iSCSI targets and NFS exports via LINSTOR. Installing LINSTOR-Gateway is a prerequisite for enabling this feature. After enabling this feature, the Gateway entry will be displayed in the left-side menu.',
    passphrase: 'Passphrase',
    gateway_mode: 'Gateway Mode',
    gateway_mode_description:
      'Installing LINSTOR-Gateway is a prerequisite for enabling this feature. And ensure that the endpoint is correctly configured to allow communication between the LINSTOR Gateway and the LINSTOR Server.',
    custom_host: 'Custom Host',
    custom_host_description:
      "When the custom host is enabled, you need to enter the LINSTOR Gateway API endpoints in the 'Custom API' section below. The default value is the LINSTOR server IP + 8337, like http://192.168.1.1:8337/. If a custom port or different IP is used, adjust the endpoint accordingly.",
    custom_api: 'Custom API',
    grafana: 'Grafana',
    grafana_dashboard: 'Grafana Dashboard',
    grafana_description:
      'Here you can configure the Grafana Dashboard. Once set up, an entry will be displayed in the left-side menu.',
    grafana_prerequisites_note:
      'To enable Grafana dashboard functionality, you need to install and run DRBD Reactor, Prometheus, and Grafana. Additionally, import the following Grafana dashboards: Node Exporter Full (https://grafana.com/grafana/dashboards/1860-node-exporter-full/) and DRBD (https://grafana.com/grafana/dashboards/14339-drbd/). Also, modify your Grafana configuration as follows:',
    grafana_prerequisites_config: `[security]
allow_embedding = true
[auth.anonymous]
enabled = true
org_role = Viewer`,
    grafana_https_notice:
      'If this GUI is opened over HTTPS, the Grafana URL below must also use HTTPS. Browsers block HTTP dashboards inside an HTTPS page. Make sure Grafana is reachable from the browser and allow_embedding is enabled.',
    grafana_url: 'Grafana URL',
    available: 'Available',
    not_available: 'Not Available',
    connect_to_grafana: 'Connect to Grafana',
    grafana_api_access: 'Grafana API Access',
    manual_configuration: 'Manual Configuration',
    cors_manual_entry: 'Could not fetch dashboards due to CORS policy. Please enter the dashboard UID manually.',
    load_dashboard_config: 'Load Dashboard Configuration',
    panel_configuration: 'Panel Configuration',
    panel_ids_detected: 'Panel IDs detected from the dashboard. Adjust if needed.',
    grafana_public_desc: 'For public Grafana: Leave API key empty',
    grafana_secured_desc: "For secured Grafana: Create a service account token with 'Viewer' role",
    grafana_api_usage: 'The API is used to auto-detect dashboards and datasources',
    no_dashboards_found: 'No dashboards found',
    no_dashboards_description:
      'No dashboards were found. Please check your Grafana server or import a dashboard first.',
    dashboard_selection: 'Dashboard Selection',
    dashboards_found: 'Found {{count}} dashboard(s). Select one to automatically extract its configuration.',
    grafana_server_url: 'Grafana Server URL',
    please_enter_grafana_url: 'Please enter Grafana URL',
    grafana_url_help:
      'Enter your Grafana server URL. Use the same protocol as this GUI. For example, if the GUI uses HTTPS, enter an HTTPS Grafana URL.',
    dashboard_url: 'Dashboard URL',
    please_enter_dashboard_url: 'Please enter dashboard URL',
    dashboard_url_help:
      'Enter the full Grafana dashboard URL. If this GUI uses HTTPS, this URL must also use HTTPS or the browser will block it.',
    invalid_dashboard_url: 'Invalid dashboard URL format. Please enter a valid Grafana dashboard URL.',
    panel_ids_default_values:
      'Default panel IDs are: CPU=77, Memory=78, Network=74, Disk=152, Disk IOPS=229, IO Usage=9',
    api_key_optional: 'API Key (Optional)',
    api_key_tooltip:
      'Required if your Grafana server has authentication enabled. Create a service account token in Grafana: Configuration → Service Accounts',
    api_key_help: 'Leave empty if your Grafana allows anonymous access',
    api_key_placeholder: 'glsa_xxxxxxxxxxxx',
    dashboard_uid: 'Dashboard UID',
    please_enter_dashboard_uid: 'Please enter dashboard UID',
    dashboard_uid_help: 'Enter the dashboard UID from Grafana URL (e.g., rYdddlPWk from /d/rYdddlPWk/node-exporter)',
    dashboard_uid_placeholder: 'e.g., rYdddlPWk',
    choose_dashboard: 'Choose a Dashboard',
    please_select_dashboard: 'Please select a dashboard',
    dashboard_selection_help: 'Select a Node Exporter or system monitoring dashboard',
    select_dashboard_placeholder: 'Select a dashboard',
    datasource: 'Datasource',
    please_select_datasource: 'Please select or enter datasource',
    datasource_help: 'Prometheus datasource ID or UID',
    select_prometheus_datasource: 'Select Prometheus datasource',
    prometheus_placeholder: 'prometheus',
    cpu_panel_id: 'CPU Panel ID',
    cpu_panel_help: 'Panel showing CPU usage',
    memory_panel_id: 'Memory Panel ID',
    memory_panel_help: 'Panel showing memory usage',
    network_panel_id: 'Network Panel ID',
    network_panel_help: 'Panel showing network traffic',
    disk_panel_id: 'Disk Usage Panel ID',
    disk_panel_help: 'Panel showing disk space usage',
    disk_iops_panel_id: 'Disk IOps Panel ID',
    disk_iops_panel_help: 'Panel showing disk operations per second',
    io_usage_panel_id: 'I/O Usage Panel ID',
    io_usage_panel_help: 'Panel showing I/O read/write bytes',
    not_detected: 'Not detected',
    step_connect: 'Connect',
    step_connect_desc: 'Connect to Grafana',
    step_connect_select_desc: 'Connect and select dashboard',
    step_select_dashboard: 'Select Dashboard',
    step_select_desc: 'Choose dashboard',
    step_configure: 'Configure',
    step_configure_desc: 'Review settings',
    connected: 'Connected',
    custom: 'Custom',
    default: 'Default',
    gateway_config_title: 'Gateway Configuration',
    gateway_config_description:
      'Configure LINSTOR Gateway settings to manage storage resources through the gateway API. Make sure to run "linstor-gateway check-health" in the terminal and resolve any issues.',
    gateway_url_required: 'Please enter gateway URL',
    gateway_connect_error: 'Cannot connect to LINSTOR-Gateway',
    gateway_url_placeholder: 'http://192.168.1.1:8337/',
    test_connection: 'Test Connection',
    linstor_passphrase: 'LINSTOR Passphrase',
    linstor_passphrase_loading: 'Loading passphrase status',
    passphrase_not_set: 'Passphrase not set',
    linstor_locked: 'LINSTOR is locked',
    linstor_unlocked: 'LINSTOR is unlocked',
    set_passphrase: 'Set Passphrase',
    unlock_passphrase: 'Unlock Passphrase',
    please_input_passphrase: 'Please input your passphrase!',
    enter_passphrase: 'Enter passphrase',
    confirm_passphrase: 'Confirm Passphrase',
    please_confirm_passphrase: 'Please confirm your passphrase!',
    passphrase_not_match: 'The two passphrases do not match!',
    confirm_passphrase_placeholder: 'Confirm passphrase',
    logo_size_error: 'The logo file size should not exceed 16KB.',
    // DRBD Dashboard
    drbd_dashboard: 'DRBD Dashboard',
    drbd_dashboard_url: 'DRBD Dashboard URL',
    drbd_panel_configuration: 'DRBD Panel Configuration',
    drbd_panel_config_description: 'DRBD dashboard panel IDs configuration',
    drbd_write_rate_panel_id: 'DRBD Write Rate Panel ID',
    drbd_write_rate_panel_help: 'Panel ID for DRBD write rate metrics',
    drbd_read_rate_panel_id: 'DRBD Read Rate Panel ID',
    drbd_read_rate_panel_help: 'Panel ID for DRBD read rate metrics',
    drbd_requires_grafana_enabled: 'Requires Grafana Dashboard to be enabled',
    please_enter_drbd_dashboard_url: 'Please enter DRBD dashboard URL',
    drbd_url_help: 'Enter the complete DRBD Grafana dashboard URL',
    invalid_drbd_dashboard_url: 'Invalid DRBD dashboard URL. Please check the format and try again.',
    loading_drbd_dashboard: 'Loading DRBD Dashboard...',
    failed_to_load_drbd_write_rate: 'Failed to load DRBD Write Rate Dashboard. Please check your configuration.',
    failed_to_load_drbd_read_rate: 'Failed to load DRBD Read Rate Dashboard. Please check your configuration.',
    drbd_dashboard_not_configured: 'DRBD Dashboard not configured or disabled',
    grafana_config_saved: 'Grafana configuration saved successfully',
    please_complete_required_fields: 'Please complete all required fields',
    grafana_dashboard_not_enabled: 'Grafana Dashboard is not enabled. Please configure it in Settings.',
    back_to_resources: 'Back to Resources',
    controller_auth: 'Controller Auth',
    controller_auth_storage_title: 'Token initialization',
    controller_auth_storage_description:
      'The initialization response returns the first token once. The GUI stores that returned token in this browser so future controller requests can include it.',
    controller_auth_initialize: 'Initialize Token Auth',
    controller_auth_already_initialized: 'Token authentication already initialized',
    controller_auth_disable: 'Disable Token Auth',
    controller_auth_disable_confirm_title: 'Disable token authentication?',
    controller_auth_disable_confirm_description:
      'The controller will stop requiring an API token, so anyone who can reach the controller gets full access. Existing tokens are kept but no longer enforced. HTTPS stays enabled and is not affected.',
    controller_auth_disabled: 'Controller token authentication disabled.',
    controller_auth_disable_failed: 'Failed to disable controller token authentication.',
    controller_auth_enter_token: 'Modify Access Token',
    controller_auth_https_switch_title: 'Controller HTTPS will be enabled',
    controller_auth_https_switch_description:
      'Initializing token authentication also enables controller HTTPS by default, so the REST endpoint switches to HTTPS. Continue from {{url}} if the current HTTP session stops responding.',
    controller_auth_initialized: 'Controller token authentication initialized.',
    controller_auth_initialized_title: 'Controller Token Auth Initialized',
    controller_auth_initialized_notice: 'Copy this token before opening the HTTPS endpoint.',
    controller_auth_initialized_notice_description:
      'The HTTPS endpoint is {{url}}. Browser localStorage is scoped by origin, so this token might need to be entered again after opening that address.',
    controller_auth_open_https: 'Open HTTPS Endpoint',
    controller_auth_stay_here: 'Stay Here',
    controller_auth_initialize_failed: 'Failed to initialize controller token authentication.',
    controller_auth_initialize_missing_token: 'Controller initialization completed but no token was returned.',
    controller_auth_already_enabled:
      'Controller token authentication is already enabled. Existing token values cannot be returned again.',
    controller_auth_already_enabled_notice:
      'Token authentication is already enabled. Use an existing token after opening the HTTPS endpoint.',
    controller_auth_no_token_notice:
      'No token was returned by the controller. Use an existing token after opening the HTTPS endpoint.',
    controller_auth_status: 'Current Status',
    controller_auth_token_present: 'A controller token is stored in this browser.',
    controller_auth_token_missing: 'No controller token is currently stored in this browser.',
    controller_auth_token: 'Controller Token',
    controller_auth_token_help: 'Paste a controller Bearer token and save it after validation.',
    controller_auth_token_required: 'Please enter the controller token.',
    controller_auth_token_placeholder: 'Paste Bearer token',
    controller_auth_save: 'Save Token',
    controller_auth_verify_saved: 'Verify Saved Token',
    controller_auth_clear: 'Clear Token',
    controller_auth_token_saved: 'Controller token saved successfully.',
    controller_auth_token_valid: 'The saved controller token is valid.',
    controller_auth_token_invalid: 'The controller token could not be verified.',
    controller_auth_token_cleared: 'Controller token cleared from this browser.',
    controller_auth_last_verified: 'Last Verification',
    controller_auth_last_verified_version: 'Controller version: {{version}}',
    controller_auth_note:
      'If controller token authentication is enabled, clearing the token will cause future controller requests to require re-authentication.',
    node_stats_title: 'Node Stats',
    dashboard_for_node: 'Dashboard for {{nodeName}}',
    node_exporter: 'Node Exporter',
    drbd_write_rate: 'DRBD Write Rate',
    drbd_read_rate: 'DRBD Read Rate',
    drbd_reactor_dashboard_info: 'You can enable DRBD Reactor dashboard in Settings page',
    go_to_settings: 'Go to Settings',
  },
  about: {
    linstor_version: 'LINSTOR Version',
    ui_version: 'UI Version',
    sds_version: 'LINBIT SDS Version',
    controller_ip: 'Controller Binding IP',
    controller_active_on: 'Controller Active On',
    trademark:
      'LINSTOR-GUI is free software: you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation, version 3 of the License. This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.',
    // Unofficial build modal
    unofficial_build_attention: 'Attention! You are using an unsupported build of this software.',
    unofficial_build_description: 'By acquiring an official version through a support subscription from LINBIT:',
    unofficial_build_benefit_support: 'You get access to the LINBIT expert support team.',
    unofficial_build_benefit_packages: 'You get access to prebuilt packages for the whole LINBIT SDS stack.',
    unofficial_build_benefit_development: 'You support the continued development of the LINBIT SDS stack.',
    unofficial_build_get_official: 'For Official Builds',
    // Header tools
    unofficial_build_header_attention: 'Attention! You are using an unsupported build.',
    unofficial_build_header_get_official: 'For Official Builds',
  },
  ha: {
    manage_ha_configuration: 'Manage HA Configuration',
    select_unmanaged_configuration_file: 'Select an unmanaged configuration file',
    list: 'DRBD Reactor Configurations',
    volume_count: 'Volumes',
    primary_node: 'Primary Node',
    config_files: 'Configuration Files',
    drbd_info: 'DRBD Info',
    edit_config: 'Edit Configuration',
    create_resource: 'Create Resource',
  },
};

export default en;
